...
int main(){_
	...
	while (scanf("%i",&n)!=EOF){
		...
		for (int i=n; i>=1; --i){
			termo = a[i];
			tamanho = sz(divs[termo]);

			menor_parcial = inicial;
			parcial = inicial - termo - a[0];

			for (int j=0; j<tamanho; ++j){
				menor_parcial = min((parcial + (termo/divs[termo][j]) + a[0]*divs[termo][j]), menor_parcial);
			}

			ans = min(ans,menor_parcial);
		}
